...
public class FeatureEnvyRefactoringStrategyTest extends LightCodeInsightFixtureTestCase {
    ...
    private static File getSourceRoot() {
        String testOutput = PathManager.getJarPathForClass(FeatureEnvyRefactoringStrategy.class);
        return new File(testOutput, "../../..");
    }

    public void testMoveMethodParametrico() {
        //Ottengo il Project dalla test Suite
        Project project = myFixture.getProject();
        //Ottengo i files
        PsiFile[] files = myFixture.configureByFiles("moveMethod/parameter/bCustomer.java", "moveMethod/parameter/bPhone.java");
        ...
        PsiClass psiSource = ((JavaPsiFacadeEx) javaPsiFacade).findClass("moveMethod.parameter.Customer");
        PsiClass psiDestination = ((JavaPsiFacadeEx) javaPsiFacade).findClass("moveMethod.parameter.Phone");
        //Ottengo i bean Oracolo
        ClassBean sourceClassBean = generateOracle(psiSource);
        ClassBean destinationClassBean = generateOracle(psiDestination);
        MethodBean methodBean = generateOracle(psiSource.findMethodsByName("getMobilePhoneNumber", true)[0], psiDestination);
        /*stampaBean(sourceClassBean);
        stampaBean(destinationClassBean);
        stampaBean(methodBean);*/
        //Creo un FeatureEnvy per il test
        FeatureEnvyRefactoringStrategy feRefactorStrategy = new FeatureEnvyRefactoringStrategy(methodBean, project);
        WriteCommandAction.runWriteCommandAction(project, () -> {
            try {
                feRefactorStrategy.doRefactor();
            } catch (FeatureEnvyException e) {
                e.printStackTrace();
            }
        });
        //Confronto il moveMethod con i files oracolo
        myFixture.checkResultByFile("moveMethod/parameter/bCustomer.java", "moveMethod/parameter/aCustomer.java", true); //CONFRONTO CON ORACOLO
        myFixture.checkResultByFile("moveMethod/parameter/bPhone.java", "moveMethod/parameter/aPhone.java", true); //CONFRONTO CON ORACOLO*/
    }

    public void testMoveMethodGlobale() {
        //Ottengo il Project dalla test Suite
        Project project = myFixture.getProject();
        //Ottengo i files
        PsiFile[] files = myFixture.configureByFiles("moveMethod/global/bCustomer.java", "moveMethod/global/bPhone.java");
        //Ottengo il JavaPsiFacade ovvero l'oggetto per fare ricerche
        JavaPsiFacade javaPsiFacade = myFixture.getJavaFacade();
        //Settiamo i psi Class
        PsiClass psiSource = ((JavaPsiFacadeEx) javaPsiFacade).findClass("moveMethod.global.Customer");
        PsiClass psiDestination = ((JavaPsiFacadeEx) javaPsiFacade).findClass("moveMethod.global.Phone");
        //Ottengo i bean Oracolo
        ClassBean sourceClassBean = generateOracle(psiSource);
        ClassBean destinationClassBean = generateOracle(psiDestination);
        MethodBean methodBean = generateOracle(psiSource.findMethodsByName("getMobilePhoneNumber", true)[0], psiDestination);
